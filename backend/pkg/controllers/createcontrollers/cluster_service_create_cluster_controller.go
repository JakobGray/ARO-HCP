...
func (c *clusterServiceCreateClusterSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	existingClusterServiceCluster, err := c.findExistingClusterServiceCluster(ctx, existingCluster)
	if err != nil {
		return utils.TrackError(fmt.Errorf("failed to search for existing CS cluster: %w", err))
	}

	if existingClusterServiceCluster == nil {
		existingClusterServiceCluster, err = c.createClusterServiceCluster(ctx, existingCluster, existingServiceProviderCluster.Spec.ControlPlaneVersion.DesiredVersion)
		if err != nil {
			return utils.TrackError(fmt.Errorf("failed to create cluster in CS: %w", err))
		}
	}

	clusterServiceID, err := api.NewInternalID(existingClusterServiceCluster.HREF())
	if err != nil {
		return utils.TrackError(fmt.Errorf("failed to parse CS cluster HREF: %w", err))
	}

	logger.Info("Storing ClusterServiceID on cluster document", "clusterServiceID", clusterServiceID.String())
	existingCluster.ServiceProviderProperties.ClusterServiceID = clusterServiceID
...
func (c *clusterServiceCreateClusterSyncer) findExistingClusterServiceCluster(ctx context.Context, cluster *api.HCPOpenShiftCluster) (*arohcpv1alpha1.Cluster, error) {
	...
	iter := c.clusterServiceClient.ListClusters(searchExpression)
